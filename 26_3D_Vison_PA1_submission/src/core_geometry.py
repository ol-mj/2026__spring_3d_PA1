...
def warpPerspective(img, H, size_warped):
    ...
    img_warped = np.zeros((height, width, channels), dtype=np.uint8)
    ...
    img_warped.reshape(-1, channels)[valid] = src[src_y_nn[valid], src_x_nn[valid]]
    mask.reshape(-1)[valid] = 255

    if grayscale:
        img_warped = img_warped[..., 0]
...
def computeH_ransac(locs1, locs2, max_iter=200, threshold=3.0):
    ...
    for _ in range(max_iter):
        ...
        proj = _project_points(H_candidate, locs1)
        ...
        candidate_inliers = errors < threshold
        count = int(np.sum(candidate_inliers))
        mean_err = float(np.mean(errors[candidate_inliers])) if count > 0 else np.inf
    ...
    if np.sum(inliers) >= 4:
        try:
            best_H = computeH_norm(locs1[inliers], locs2[inliers])
        except Exception:
            pass

    return _normalize_h(best_H), inliers
